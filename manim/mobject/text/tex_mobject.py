...
def tex_to_svg(tex_string: str) -> str:
    """Convert a TeX string to an SVG string.
    
    Uses MathJax to convert the TeX string to an SVG string."""
    MathJax = window.MathJax
    node = MathJax.tex2svg(
        tex_string,
        display=True,
    )
    content = node.innerHTML
    idx = content.find("</svg>")
    content = content[:idx + 6]  # include </svg> tag
    return content
...
class SingleStringMathTex(SVGMobject):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        content = tex_to_svg(self._get_modified_expression(tex_string))
        super().__init__(
            content=content,
